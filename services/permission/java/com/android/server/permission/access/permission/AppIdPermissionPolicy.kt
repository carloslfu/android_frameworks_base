...
class AppIdPermissionPolicy : SchemePolicy() {
    ...
    private fun MutateStateScope.evaluateAllPermissionStatesForPackageAndUser(
        ...
        if (androidPackage != null) {
            val packageHooks = PackageExt.get(androidPackage).hooks()
            androidPackage.requestedPermissions.forEach { permissionName ->
                val appId = packageState.appId
                evaluatePermissionState(appId, userId, permissionName, installedPackageState)

                val override = packageHooks.overridePermissionState(permissionName, userId)
                var flags = -1
                if (override == PackageHooks.PERMISSION_OVERRIDE_GRANT) {
                    ...
                    if (
                        !getPermissionFlags(appId, userId, permissionName)
                            .hasBits(PermissionFlags.USER_SET)
                    ) {
                        flags = 0
                    }
                }
