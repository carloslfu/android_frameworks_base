...
class ActiveServicesHooks {
    ...
    static boolean shouldAllowFgsWhileInUsePermission(
            ActiveServices activeServices, int uid) {
        ...
        boolean res =
                PackageExt.get(pkg)
                        .hooks()
                        .shouldAllowFgsWhileInUsePermission(pm, UserHandle.getUserId(uid));
        if (res) {
            Slog.d(
                    TAG,
                    "shouldAllowFgsWhileInUsePermission for "
                            + pkg.getPackageName()
                            + " returned true");
        }
        return res;
