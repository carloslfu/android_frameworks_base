...
public class PermissionManagerService extends IPermissionManager.Stub {
    ...
    public void updatePermissionStateAndInvalidateCache(String packageName, int userId) {
        ...
        if (UserHandle.getUserId(callingUid) != userId) {
            ...
            Slog.w(LOG_TAG, "updatePermissionStateAndInvalidateCache: cross user granted for "
                    + callingUid);
        }
        ...
        if (packageState == null) {
            Slog.w(LOG_TAG, "updatePermissionStateAndInvalidateCache: no PackageState for "
                    + packageName);
            return;
        }
        ...
        try {
            ...
            if (appInfo == null) {
                Slog.w(LOG_TAG, "updatePermissionStateAndInvalidateCache: no appInfo for "
                        + packageName);
                return;
            }
            ...
            Slog.e(LOG_TAG, "updatePermissionStateAndInvalidateCache: failed for "
                    + packageName, e);
        } finally {
            Binder.restoreCallingIdentity(identity);
        }
