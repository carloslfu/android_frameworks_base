...
public class PackageHooks {
     ...
     *     package to self
     */
    public boolean shouldBlockPackageVisibility(
            int userId, PackageStateInternal otherPkg, boolean isSelfToOther) {
    ...
    public static boolean shouldBlockAppsFilterVisibility(
        ...
        if (callingPkgSetting != null) {
            return shouldBlockPackageVisibilityTwoWay(
                    callingPkgSetting, callingUserId, targetPkgSetting, targetUserId);
        }

        for (int i = callingSharedPkgSettings.size() - 1; i >= 0; --i) {
            boolean res =
                    shouldBlockPackageVisibilityTwoWay(
                            callingSharedPkgSettings.valueAt(i),
                            callingUserId,
                            targetPkgSetting,
                            targetUserId);
            if (res) {
    ...
    private static boolean shouldBlockPackageVisibilityTwoWay(
            ...
            int pkgUserId,
            PackageStateInternal otherPkgSetting,
            int otherPkgUserId) {
        boolean res =
                shouldBlockPackageVisibilityInner(
                        pkgSetting, pkgUserId, otherPkgSetting, true);
        if (!res) {
            res =
                    shouldBlockPackageVisibilityInner(
                            otherPkgSetting, otherPkgUserId, pkgSetting, false);
        }
        return res;
    }
    ...
    public boolean shouldAllowFgsWhileInUsePermission(
            PackageManagerInternal pm, int userId) {
        return false;
    }
}
